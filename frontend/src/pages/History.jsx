...
export default function History() {
  ...
  const filtered = analyses.filter((a) => {
    ...
    const matchSearch =
      !search ||
      (a.rawText || "").toLowerCase().includes(search.toLowerCase());
  ...
  const stats = {
    ...
    medium: analyses.filter((a) => {
      const s = a.result?.overallRiskScore ?? 0;
      return s > 3 && s <= 6;
    }).length,
  ...
  return (
    <div className="page">
      ...
      >
        ...
        {/* Search + Filter */}
        <div className="history-search-filter-wrapper">
          <input
        ...
        {loading ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="skeleton"
                style={{ height: 90, borderRadius: "var(--radius-lg)" }}
              />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          ...
          >
            ...
            <div style={{ fontSize: 13, color: "var(--text-muted)" }}>
              {analyses.length === 0
                ? "Upload your first offer letter from the Dashboard."
